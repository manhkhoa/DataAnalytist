...
.controller('MainCtrl', ['$scope', '$http', 'version', '$location',
  function($scope, $http, version, $location) {
    ...
    $http({
      method: 'POST',
      url: "/sites/all/modules/custom/hirenchart/app/data/results.json"
      // url: "/sites/all/modules/custom/hirenchart/app/data/test.json"
    }).
...
.controller('FccCtrl', ['$scope', '$http', 'version', '$location',
  function($scope, $http, version, $location) {
    ...
    $http({
      method: 'GET',
      url: "http://localhost/data_analytis/sites/all/modules/custom/chart/app/data/fccdata.json"
    }).
    success(function(data, status, headers, config) {
      // Do a little minipulation of the data formatting to create a valid root
      var root = {
